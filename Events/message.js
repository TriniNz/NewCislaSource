exports.run = async (Discord, thisClient, message, db, dbcmd, cooldown) => {
    ...
    if(message.content.indexOf(">") == 0 && message.channel.id == 602687787141038103) {message.react('✅'); message.react('❎')}
    ...
    if(message.content.indexOf(prefix) == 0) { 
        try {
            ...
            if(cmdSearch.manutenção) return require('../Commands/Manutenção.js').run(Discord, thisClient, messsage, args, db);
